...
transformed parameters{
  ...
  for (i in 1:NumDatapoints){
    for(j in 1:IntDim){
      random_effects[i] += lambda[i,j] + intercept[LTLAs[i]]; // lambda * gamma^T in manuscript. Note use of intercept makes this line akin to IntDim (B) = 2 with column vector of 1s for one column of lambda
    }
  }
